...
fn load_data(path: &str) -> Result<(Array2<f32>, Vec<usize>), Box<dyn Error>> {
    ...
    for result in rdr.records() {
        ...
        let label = match &record[record.len() - 1] {
            "WALKING" => 0,
            "WALKING_UPSTAIRS" => 1,
            ...
            "STANDING" => 4,
            "LAYING" => 5,
            _ => panic!("Unknown label: '{}'", &record[record.len() - 1]),
        };
...
impl NeuralNetwork {
    fn new(input_size: usize, hidden1_size: usize, hidden2_size: usize, output_size: usize, learning_rate: f32) -> Self {
        Self {
            w_input_hidden1: Array2::random((input_size, hidden1_size), Uniform::new(-0.5, 0.5)),
            w_hidden1_hidden2: Array2::random((hidden1_size, hidden2_size), Uniform::new(-0.5, 0.5)),
            w_hidden2_output: Array2::random((hidden2_size, output_size), Uniform::new(-0.5, 0.5)),
            learning_rate,
        }
    }
    ...
    fn softmax(x: &Array2<f32>) -> Array2<f32> {
        ...
        for (mut out_row, in_row) in result.outer_iter_mut().zip(x.outer_iter()) {
            let max = in_row.iter().copied().fold(f32::NEG_INFINITY, f32::max);
            let exps: Vec<f32> = in_row.iter().map(|v| (*v - max).exp()).collect();
            let sum: f32 = exps.iter().sum();
            for (i, val) in out_row.iter_mut().enumerate() {
                *val = exps[i] / sum;
            }
        }
        result
    ...
    fn forward(&self, x: &Array2<f32>) -> (Array2<f32>, Array2<f32>, Array2<f32>) {
        let a1 = Self::sigmoid(&x.dot(&self.w_input_hidden1));
        let a2 = Self::sigmoid(&a1.dot(&self.w_hidden1_hidden2));
        let a3 = Self::softmax(&a2.dot(&self.w_hidden2_output));
        (a1, a2, a3)
    }
    ...
    fn backward(&mut self, x: &Array2<f32>, a1: &Array2<f32>, a2: &Array2<f32>, a3: Array2<f32>, target: &[usize]) {
        ...
        self.w_hidden2_output -= &a2.t().dot(&delta_output).mapv(|v| v * self.learning_rate);
        self.w_hidden1_hidden2 -= &a1.t().dot(&delta_hidden2).mapv(|v| v * self.learning_rate);
        self.w_input_hidden1 -= &x.t().dot(&delta_hidden1).mapv(|v| v * self.learning_rate);
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    let (train_inputs, train_labels) = load_data("src/train.csv")?;
    let (test_inputs, test_labels) = load_data("src/test.csv")?;
